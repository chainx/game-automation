...
def get_address_value(address_name, process=None, psx_base=None, target="psxfin.exe", verbose=False):
    entry = ADDRESSES.get(address_name)
    if not entry:
...
def open_process(pid):
    h = Kernel.OpenProcess(0x0400 | 0x0010, 0, pid)  # QUERY_INFORMATION | VM_READ
...
def read_mem(h, addr, size):
    ...
    if not (Kernel.ReadProcessMemory(h, ct.c_void_p(addr), ct.byref(buf), size, ct.byref(rd)) and rd.value == size):
...
def aob_scan_first(h, pat, chunk=8*1024*1024, max_scan_seconds=15, verbose=False):
    mbi, a = MemoryBasicInformation(), 0
    ...
    while Kernel.VirtualQueryEx(h, ct.c_void_p(a), ct.byref(mbi), ct.sizeof(mbi)):
        ...
        if st == 0x1000 and (pr & 0xFF) in READABLE and not (pr & 0x100) and not (pr & 1) and sz:
            ...
            while p < end:
                ...
                if Kernel.ReadProcessMemory(h, ct.c_void_p(p), ct.byref(buf), to, ct.byref(rd)) and rd.value:
